...
// Assign buffer memory and settings.
void ringbuffer_init(ringbuffer_t* buffer, size_t  element_max, size_t element_size, void* storage);
...
// Add element to buffer. Pops oldest element if buffer is full
...
// peek to element, return pointer to it.
void* ringbuffer_peek_at(ringbuffer_t* buffer, size_t index);
