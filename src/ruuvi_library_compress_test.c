                                              ...
                                              timestamp_t *p_start_timestamp)
{
  ...
  for (uint16_t i=0; i<RL_COMPRESS_TEST_BLOCK_SIZE_DEFAULT; i+=RL_COMPRESS_TEST_DATA_SIZE_DEFAULT)
  {   
                                        ...
                                        uint8_t * p_block)
{
  bool result =  true;
  rl_data_t decompress_result;
  for (uint16_t i=start_offset; i<RL_COMPRESS_TEST_BLOCK_SIZE_DEFAULT; i+=RL_COMPRESS_TEST_DATA_SIZE_DEFAULT)
  {                  
...
static bool ruuvi_library_test_null()
{
  ...
  if (RL_COMPRESS_ERROR_NULL != rl_decompress(&find_data,NULL, RL_COMPRESS_TEST_BLOCK_SIZE_DEFAULT, 
                                              &htab, &start_timestamp))
  {
    result = false;
  }
