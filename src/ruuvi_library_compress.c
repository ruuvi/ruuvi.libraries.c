...
#     endif
         ...
         || (RL_COMPRESS_START == state->compress_state))
      {
          ...
          const size_t threshold_increment = RL_COMPRESS_COMPRESS_SIZE - cmpr_len - RL_COMPRESS_OVERHEAD;
                          ...
                          timestamp_t * start_timestamp)
{
    ...
    if(RL_COMPRESS_SUCCESS == state->compress_state)
    {
         ...
         while(((uint8_t*)state->next_sample < (state->decompress_block + RL_COMPRESS_DECOMPRESS_SIZE))
                && (state->next_sample->time < *start_timestamp))
         {
             state->next_sample++;
         }
         // If sample was not found, return error
         if((uint8_t*)state->next_sample >= (state->decompress_block + RL_COMPRESS_DECOMPRESS_SIZE))
         {
         ...
         else
         {
           ...
           *start_timestamp = data->time;
           ...
           if((uint8_t*)state->next_sample == (state->decompress_block + state->decompressed_size))
           {
